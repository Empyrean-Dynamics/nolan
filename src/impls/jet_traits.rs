use crate::jets::{Jet1, Jet2, Jet3, hess_index, tens_index};
use crate::traits::{
    AutoDiff, Differentiable, DifferentiableMath, FirstOrder, SecondOrder, ThirdOrder,
};
...
impl<const N: usize, const H: usize, const T: usize> Differentiable for Jet3<N, H, T> {
    fn value(&self) -> f64 {
        self.value
    }
...
impl<const N: usize, const H: usize, const T: usize> FirstOrder for Jet3<N, H, T> {
    fn grad(&self, i: usize) -> f64 {
        self.grad[i]
    }
...
impl<const N: usize, const H: usize, const T: usize> SecondOrder for Jet3<N, H, T> {
    fn hess(&self, i: usize, j: usize) -> f64 {
        self.hess[hess_index(i, j).unwrap()]
    }
...
impl<const N: usize, const H: usize, const T: usize> ThirdOrder for Jet3<N, H, T> {
    fn tens(&self, i: usize, j: usize, k: usize) -> f64 {
        self.tens[tens_index(i, j, k).unwrap()]
    }

    fn tens_mut(&mut self, i: usize, j: usize, k: usize) -> &mut f64 {
        &mut self.tens[tens_index(i, j, k).unwrap()]
    }
}

// ─── Jet3: DifferentiableMath ──────────────────────────────────

impl<const N: usize, const H: usize, const T: usize> DifferentiableMath for Jet3<N, H, T> {
    fn sin(self) -> Self {
        self.sin()
    }
    fn cos(self) -> Self {
...
impl<const N: usize, const H: usize, const T: usize> AutoDiff for Jet3<N, H, T> {}
